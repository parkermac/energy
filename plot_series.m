...
if 0
    ...
    plot(day,Z_jfilt(P2.pdt',nfilt)/info.mass,'--b', ...
        ...
        'linewidth',lw)
    title(['APE^{\prime} Rates [W kg^{-1}]  ::  ',upper(island_tag),' (',num2str(nfilt),' day filter)'])
...
title('Wind and River'); xlim(dlim); grid on
ylim([-.1 .3]);
[xt,yt] = Z_lab('ul');
...
[xt,yt] = Z_lab('ur');
text(xt,yt,[rivername,' River [/3e4 m^{3} s^{-1}]'],'color','b', ...
